...
public class TCWorldChunkManager extends WorldChunkManager implements
		IBiomeManager {
	...
	public TCWorldChunkManager(final SingleWorld world) {
		...
		biomesToSpawnIn
				.add(BiomeGenBase.biomeList[DefaultBiome.DESERT_HILLS.Id]);
		biomesToSpawnIn
				.add(BiomeGenBase.biomeList[DefaultBiome.FOREST_HILLS.Id]);
		biomesToSpawnIn.add(BiomeGenBase.biomeList[DefaultBiome.JUNGLE.Id]);
		biomesToSpawnIn
				.add(BiomeGenBase.biomeList[DefaultBiome.JUNGLE_HILLS.Id]);
	...
	@SuppressWarnings("rawtypes")
	@Override
	public boolean areBiomesViable(final int paramInt1, final int paramInt2,
			final int paramInt3, List paramList) {
		// Hack for
		...
		final int n = k - i + 1;
		final int i1 = m - j + 1;
		final BiomeGenBase[] arrayOfInt = getBiomesForGeneration(null, i, j, n,
				i1);
	...
	@Override
	@SuppressWarnings("rawtypes")
	public ChunkPosition findBiomePosition(final int paramInt1,
			final int paramInt2, final int paramInt3, final List paramList,
		...
		for (int i3 = 0; i3 < arrayOfInt.length; i3++) {
			...
			if (!paramList.contains(localBiomeBase)
					|| localChunkPosition != null
					&& paramRandom.nextInt(i2 + 1) != 0)
				continue;
	...
	@Override
	public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] paramArrayOfBiomeBase,
			final int paramInt1, final int paramInt2, final int paramInt3,
			final int paramInt4, final boolean paramBoolean) {
		if (paramArrayOfBiomeBase == null
				|| paramArrayOfBiomeBase.length < paramInt3 * paramInt4)
			paramArrayOfBiomeBase = new BiomeGenBase[paramInt3 * paramInt4];

		if (paramBoolean && paramInt3 == 16 && paramInt4 == 16
				&& (paramInt1 & 0xF) == 0 && (paramInt2 & 0xF) == 0) {
			synchronized (lockObject) {
				final BiomeGenBase[] localObject = cache.getCachedBiomes(
						paramInt1, paramInt2);
				System.arraycopy(localObject, 0, paramArrayOfBiomeBase, 0,
						paramInt3 * paramInt4);
			}
			return paramArrayOfBiomeBase;
		}
		final int[] localObject = biomeLayer.Calculate(paramInt1, paramInt2,
				paramInt3, paramInt4);
	...
	public BiomeGenBase[] getBiomesForGeneration(
			BiomeGenBase[] paramArrayOfBiomeBase, final int paramInt1,
			final int paramInt2, final int paramInt3, final int paramInt4) {
		if (paramArrayOfBiomeBase == null
				|| paramArrayOfBiomeBase.length < paramInt3 * paramInt4)
			paramArrayOfBiomeBase = new BiomeGenBase[paramInt3 * paramInt4];

		final int[] arrayOfInt = unZoomedLayer.Calculate(paramInt1, paramInt2,
				paramInt3, paramInt4);
		for (int i = 0; i < paramInt3 * paramInt4; i++)
	...
	@Override
	public int[] getBiomesTC(int[] biomeArray, final int x, final int z,
	...
	@Override
	public int[] getBiomesUnZoomedTC(int[] biomeArray, final int x,
	...
	// rain
	@Override
	public float[] getRainfall(float[] paramArrayOfFloat, final int paramInt1,
			final int paramInt2, final int paramInt3, final int paramInt4) {
		if (paramArrayOfFloat == null
				|| paramArrayOfFloat.length < paramInt3 * paramInt4)
			paramArrayOfFloat = new float[paramInt3 * paramInt4];

		final int[] arrayOfInt = biomeLayer.Calculate(paramInt1, paramInt2,
				paramInt3, paramInt4);
		for (int i = 0; i < paramInt3 * paramInt4; i++) {
	...
	// Temperature
	@Override
	public float[] getTemperatures(float[] paramArrayOfFloat,
			final int paramInt1, final int paramInt2, final int paramInt3,
			final int paramInt4) {
		if (paramArrayOfFloat == null
				|| paramArrayOfFloat.length < paramInt3 * paramInt4)
	...
	@Override
	public float[] getTemperaturesTC(final int x, final int z,
			final int x_size, final int z_size) {
